...
- **Google Maps API key:** ya está cargada en el código. Debe seguir con **Maps JavaScript API** y **Geocoding API** habilitadas, facturación activa y dominio autorizado. Geocoding API es lo que convierte la dirección escrita por el operador en coordenadas para ubicar la visita en el mapa. Para dibujar la ruta azul por calles entre la ubicación actual del supervisor (punto A) y la dirección aceptada (punto B), la clave también debe permitir el servicio de direcciones/rutas de Google Maps usado por `DirectionsService`.
- **Permiso de ubicación del celular:** el supervisor debe aceptar la ubicación del navegador; en producción la web debe abrirse por `https://` para que el GPS funcione correctamente. En iPhone conviene abrir desde Safari; si se abre dentro de WhatsApp, iOS puede usar el permiso de WhatsApp y no mostrar el cartel del navegador. Revisá Ajustes → Safari/WhatsApp → Ubicación. La app ahora guarda puntos como máximo cada `minRouteSeconds`, sólo si se movió al menos `minRouteMeters` y sólo con precisión aceptable, y las actualizaciones de GPS redibujan sólo marcadores/ruta sin reconstruir toda la pantalla. La precisión final depende del GPS/señal del celular; para pegar el recorrido exactamente a calles se puede evaluar más adelante Google Roads API.
...
- **Vista informativa para líderes:** muestra GPS actual del Supervisor de Calle, precisión, última señal, estado online y batería si el navegador del celular soporta `navigator.getBattery()`. No muestra acciones operativas como cerrar tarea o cerrar jornada.
...
4. **Ruta automática por cercanía:** cuando el supervisor ya tiene varias visitas aceptadas, el mapa arma automáticamente una ruta azul desde su ubicación actual pasando primero por los puntos pendientes más cercanos. Los pines se numeran según ese orden sugerido.
5. **Inicio de visita:** el supervisor puede tocar **Comenzar esta tarea** en la tarjeta o **Iniciar recorrido** desde el pin rojo numerado. Ahí se crea una ruta `fieldRoutes`, se toma GPS actual como punto A y la dirección como punto B.
...
15. El `Supervisor de Calle` es el único que ve el mapa operativo. Ve esas visitas, puede **Aceptar visita** o **Denegar**. Si acepta, queda `Pendiente`; cuando toca **Comenzar esta tarea**, el mapa toma su ubicación GPS actual como punto A y la dirección aceptada como punto B. En ese mapa puede alternar **Mapa/Satélite**, usar **Centrar ubicación** para volver a su posición en vivo, y tocar **Ir** para cargar la guía/ruta dentro del mismo mapa. Los pines pendientes aparecen en rojo, las tareas en curso en azul y las realizadas en verde; al tocar un pin pendiente se puede **Iniciar recorrido** desde el mapa, y una tarea en curso se puede **Cerrar tarea** desde el pin o desde el historial. Si hay varias visitas aceptadas, el mapa ordena automáticamente los puntos por cercanía desde la ubicación actual del supervisor y dibuja la ruta azul con ese orden sugerido. Para navegación paso a paso con voz se debe abrir Google Maps del celular.
