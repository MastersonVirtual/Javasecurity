...
4. ID del proyecto: podés dejar el automático o usar algo como `internamatutino` si está disponible.
...
5. Copiá el objeto `firebaseConfig` completo que te muestra Firebase.
...
1. `projectId` no está vacío en `index.html`.
2. Firestore Database está creado, no solo el proyecto Firebase.
3. Las reglas están publicadas y permiten `create` en `messages` y `privateMessages`.
4. La consola del navegador no muestra `Missing or insufficient permissions`.
5. La consola del navegador no muestra errores de dominio/API key.
6. Estás mirando el mismo proyecto cuyo `projectId` pegaste en la web.
